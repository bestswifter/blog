...
模块稳定性在一般情况下总是能被满足的，但是一旦涉及到内联相关的优化，就需要单独考虑了。
...
因此，（在笔者看来）模块稳定性讨论的，其实就是 `@inlinable` 内联带来的性能优化与后续升级迭代之间的权衡问题。
...
`-enable-library-evolution` 选项会以模块兼容的方式编译代码，将 `@inlinable` 这类函数用到的变量特殊处理，从直接的内存偏移修改为间接访问。
...
![](../pictures/swift-interface/inlinable-stability.png)

其中的 `rax` 来自于 `age.setter` 这个函数的偏移

![](../pictures/swift-interface/inlinable-age-setter.png)
...
![](../pictures/swift-interface/inlinable-none-stability.png)
...
后一个参数的作用就是生成本文的主角： `.swiftinterface` 文件。上一节的代码编译后产出的 `.swiftinterface` 文件如下：
...
当我们在客户端代码中引入 `Person` 这个模块时，可能已经过去了很久，编译器的版本也发生了更新。此时编译器首先会检查第三行注释中有没有 `-enable-library-evolution` 编译参数，如果没有，那么就会判断生成这个 `.swiftinterface` 文件的编译期版本，和消费的编译器版本是否一致。如果不一致就会报编译错误：

![](../pictures/swift-interface/error.png)
...
2. 高版本的 Swift 编译器可能会用到一些低版本编译器不支持的特性。因此模块兼容性不是无条件的，它要求消费方的编译器版本不低于生产方的版本。
